...
test('startPairing builds a complete_pair whose proof the daemon accepts', () => {
  ...
  const payload = { v: 1, addr: 'ws://127.0.0.1:7443', pk: exportPublic(daemon.publicKey), s: 'sekret' };
  ...
  assert.equal(attempt.target, 'ws://127.0.0.1:7443');
...
test('a non-WebSocket daemon address yields a null target (no reachable listener)', () => {
  const payload = { v: 1, addr: 'tcp://100.64.0.1:7443', pk: 'x', s: 'y' };
  assert.equal(startPairing(payload, 'phone').target, null, 'a tcp:// address is not reachable from Expo Go');
});
...
test('the persisted record alone reconstructs a session the daemon can open', () => {
  ...
  assert.equal(record.daemonAddress, 'ws://127.0.0.1:7443', 'the record remembers the WebSocket URL to reconnect to');
